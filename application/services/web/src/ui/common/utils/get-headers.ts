...
export const getHeaders = () => {
  ...
  return {
    ...headers,
    /**
     * When an application is running on local machine(localhost) and not on k8s, we should use API_HOST
     * to send requests to the server.
     * Because headers.host in this case localhost:3000, but ingress server deployed on k8s with API_HOST
     */
